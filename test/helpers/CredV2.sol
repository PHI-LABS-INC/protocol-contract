...
contract CredV2 is Initializable, UUPSUpgradeable, Ownable2StepUpgradeable, PausableUpgradeable, ICred {
    ...
    uint256 public constant SHARE_LOCK_PERIOD = 100; //Block range for avoiding flush loan
    ...
    mapping(uint256 credId => mapping(address curator => uint256 blocknumber)) public lastTradeBlock;
        ...
        whenNotPaused
    {
        ...
        if (sender != _msgSender()) revert UnauthorizedCaller();

        if (!curatePriceWhitelist[bondingCurve]) revert UnauthorizedCaller();
        ...
        whenNotPaused
    {
        ...
        if (creds[credId].creator != _msgSender()) revert UnauthorizedCaller();
        if (buyShareRoyalty_ > MAX_ROYALTY_RANGE || sellShareRoyalty_ > MAX_ROYALTY_RANGE) {
            revert InvalidRoyaltyRange();
        }
        ...
        emit CredUpdated(_msgSender(), credId, credURL);
    }
        ...
        returns (uint256)
    {
        ...
        emit CredCreated(creator_, credIdCounter, credURL_, credType_, verificationType_, 1);
        ...
        whenNotPaused
    {
        ...
        if (isBuy) {
            ...
            if (block.number <= lastTradeBlock[credId_][curator_] + SHARE_LOCK_PERIOD) {
                revert ShareLockPeriodNotPassed(block.number, lastTradeBlock[credId_][curator_] + SHARE_LOCK_PERIOD);
            }
        ...
        if (isBuy) {
            ...
            lastTradeBlock[credId_][curator_] = block.number;
        } else {
            cred.currentSupply -= amount_;
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < credIds_.length; ++i) {
            ...
            if (isBuy) {
                ...
                lastTradeBlock[credId][curator] = block.number;
            } else {
                if (block.number <= lastTradeBlock[credId][curator] + SHARE_LOCK_PERIOD) {
                    revert ShareLockPeriodNotPassed(block.number, lastTradeBlock[credId][curator] + SHARE_LOCK_PERIOD);
                }
                creds[credId].currentSupply -= amount;
            }
    ...
    /*//////////////////////////////////////////////////////////////
                                INTERNAL
    //////////////////////////////////////////////////////////////*/
    function _recoverSigner(bytes32 hash_, bytes memory signature_) private pure returns (address) {
        bytes32 cr;
        bytes32 cs;
        assembly {
            cr := mload(add(signature_, 32))
            cs := mload(add(signature_, 64))
        }

        return ECDSA.recover(MessageHashUtils.toEthSignedMessageHash(hash_), cr, cs);
    }
